...
bool generateAndSaveMap(int nodeCount, std::string filePath) {
    ...
    for (int i = 0; i < nodeCount; i++) {
        ...
        if (i < nodeCount * 0.6) { // 核心密集的市中心居民区
            ...
            std::normal_distribution<> disNormX(centers[centerIdx].first, MAP_SIZE * 0.08);
            std::normal_distribution<> disNormY(centers[centerIdx].second, MAP_SIZE * 0.08);
    ...
    std::cout << "\nDataGenerator: 正在使用 Prim 算法构建包含 " << nodeCount << " 个点的全国主干快线骨架..." << std::endl;

    // 【2】釜底抽薪：利用 Prim (最小生成树) 进行数学上不留一丝遗憾的全局并网！
    // 采用 O(V^2) 双重循环阵列，C++ 几百毫秒内即可跑完一亿次判定
    ...
    for (int count = 0; count < nodeCount; ++count) {
        ...
        // 步骤 c：由于这名新成员的加入，刷新一下所有还流落在外的孤点与树干的最短联系成本
        for (int v = 0; v < nodeCount; ++v) {
            if (!inMST[v]) {
    ...
    std::cout << "DataGenerator: 最小生成树(MST)骨干结界架构完成！已实现完全连通绝无任何死点！" << std::endl;
    ...
    for (int i = 0; i < nodeCount; ++i) {
        int gx = std::clamp(static_cast<int>(nodes[i].x / CELL_SIZE), 0, gridSize - 1);
        int gy = std::clamp(static_cast<int>(nodes[i].y / CELL_SIZE), 0, gridSize - 1);
    ...
    for (int i = 0; i < nodeCount; ++i) {
        ...
        while (neighbors.size() < k_redundancy + 1 && searchRadius < 7) {
            ...
            if (neighbors.size() >= k_redundancy) break;
            searchRadius++; 
        }
        ...
        for (int m = 0; m < max_check && addedRedundant < k_redundancy; ++m) {
            ...
            if (added) addedRedundant++;
        }
    }

    std::cout << "DataGenerator: 所有普通城内街道分发完毕。共搭载 " << nodeCount << " 节点与 " 
              << edgeIdCounter << " 条双向通道。开始生成 Json ..." << std::endl;
